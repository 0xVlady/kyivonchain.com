          ...
          <div className="space-y-8 sm:space-y-12">
            ...
            <div className="glass-card rounded-2xl p-6 sm:p-8 w-full">
              ...
              <div className="prose prose-base sm:prose-lg max-w-none text-foreground w-full">
                ...
                <p className="mb-4 sm:mb-6 break-words">
                  {t('vision.solana.grants').split(/(\[GRANTS_LINK\]|\[KUMEKA_GRANTS_LINK\])/).map((part, index) => {
                  if (part === '[GRANTS_LINK]') {
                    return <a key={index} href="https://solana.org/grants-funding" target="_blank" rel="noopener noreferrer" className="text-primary hover:text-primary/80 underline">
                            {language === 'en' ? 'Solana Foundation Grants program' : 'програму Solana Foundation Grants'}
                          </a>;
                  }
                  if (part === '[KUMEKA_GRANTS_LINK]') {
                    return <a key={index} href="https://earn.superteam.fun/grants/kumeka-team-instagrants" target="_blank" rel="noopener noreferrer" className="text-primary hover:text-primary/80 underline">
                ...
                <p className="mb-4 sm:mb-6 break-words font-medium">
                  {t('vision.accelerators.list').split(/(\[COLOSSEUM_LINK\]|\[SOLANA_LABS_LINK\]|\[VENTURE_LAUNCH_LINK\]|\[OUTLIER_VENTURES_LINK\])/).map((part, index) => {
                  if (part === '[COLOSSEUM_LINK]') {
                    return <a key={index} href="https://www.colosseum.com/accelerator" target="_blank" rel="noopener noreferrer" className="text-primary hover:text-primary/80 underline">
                            Colosseum
                          </a>;
                  }
                  if (part === '[SOLANA_LABS_LINK]') {
                    return <a key={index} href="https://incubator.solanalabs.com/" target="_blank" rel="noopener noreferrer" className="text-primary hover:text-primary/80 underline">
                            Solana Labs Incubator
                          </a>;
                  }
                  if (part === '[VENTURE_LAUNCH_LINK]') {
                    return <a key={index} href="https://venturelaunch.xyz/" target="_blank" rel="noopener noreferrer" className="text-primary hover:text-primary/80 underline">
                            Venture Launch
                          </a>;
                  }
                  if (part === '[OUTLIER_VENTURES_LINK]') {
                    return <a key={index} href="https://outlierventures.io/base-camp/" target="_blank" rel="noopener noreferrer" className="text-primary hover:text-primary/80 underline">
